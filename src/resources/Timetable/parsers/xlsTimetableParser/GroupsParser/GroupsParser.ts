import { type Group, GroupType, type GroupTypeValues, type ParserInterfaceV2, type RowType } from '@types';
import type { GroupRegexesConfig, GroupsMapKey, GroupsParserArgs, GroupsParserReturn } from './types';

export class GroupsParser implements ParserInterfaceV2<GroupsParserArgs, GroupsParserReturn> {
  static determineGroupType(text: string, regexesConfig?: Partial<GroupRegexesConfig>): GroupTypeValues {
    const combineRegexes = (regex: RegExp | RegExp[], regexesFromConfig?: RegExp[]) => {
  ...
  #createGroupExtractor() {
    ...
    return (columnIndex?: number, exerciseGroupNumber?: string, groupYear?: string, hasTwoLabGroups?: boolean) => {
      ...
      const key = (groupType: GroupTypeValues, providedColumnIndex?: number): GroupsMapKey =>
        `${groupType}-${providedColumnIndex ?? columnIndex}`;
      ...
      if (!mappedExerciseGroups.includes(exerciseGroupId)) {
        ...
        const exerciseGroup = this.#createGroup(exerciseGroupNumber, GroupType.EXERCISE, groupYear);
        const firstLabGroup = this.#createGroup(labGroupNumber - 1, GroupType.LABORATORY, groupYear);
        const secondLabGroup = this.#createGroup(labGroupNumber, GroupType.LABORATORY, groupYear);
  ...
  #createGroup(number: number | string, type: GroupTypeValues, year: number | string): Group {
    return {
